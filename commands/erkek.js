...
module.exports = {
	...
	async execute(message,args) {
        this.usage=`Kullanıcının verilerini eksik girdin Örnek kullanım:${config.prefix}e MEHA 1`
        ...
        userparser(args[0],message.guild.id).then(u => {
            if (!u) {
            ...
            else if (message.guild.members.cache.get(u.id).roles.cache.has(' 837262737351770112') || message.guild.members.cache.get(u.id).roles.cache.has('837262738312527883')) {
                if(!message.author.bot){
                 const emb = new discord.MessageEmbed()
                .setTitle(`${message.guild.name} Kayıt Sistemi`)
                .setColor('RED')
                ...
                client.guilds.cache.get(message.guild.id).members.cache.get(u.id).roles.add('837262737351770112',`${message.author.username} Tarafından Kayıt Edildi`)
                }
if(!message.author.bot){
